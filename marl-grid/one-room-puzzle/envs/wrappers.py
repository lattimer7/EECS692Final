...
def normalize_dict_obs(obs_dict):
    for k, v in obs_dict.items():
        ...
        if isinstance(v, dict):
            obs_dict[k]['pov'] = np.maximum((v['pov'] / 255.), 1e-4)
        else:
            obs_dict[k] = np.maximum((v / 255.), 1e-4)
    return obs_dict
...
class GridWorldEvaluatorWrapper(gym.Wrapper):
    ...
    def render_reward(self, frame, reward_dict, info_dict):
        if self.show_reward:
            ...
            # render reward
            reward_dict.pop('step_rewards', None)
            reward_dict.pop('env_rewards', None)
            reward_dict.pop('comm_rewards', None)
            to_render += ['rew',
