...
class Worker(mp.Process):
    ...
    def get_trajectory(self, hidden_state, state_var, done):
        ...
        while not check_done(done) and len(trajectory[0]) < self.t_max:
            plogit, value, hidden_state, comm_out, ae_lstm_loss = self.net(
            ...
            trajectory[0].append((plogit, action, value, reward, ae_lstm_loss))
        ...
        while not self.master.is_done():
            ...
            for aid in range(self.num_acts):
                ...
                for agent in self.agents:
                    ...
                    pls, vls, els = [], [], []
                    for i, (pi_logit, action, value, reward, (ae_loss, lstm_loss)
                            ) in enumerate(traj):
            ...
            progress_str = self.pfmt.format(
                ...
                np.around(np.mean(ae_losses), decimals=5),
